$(document).ready(function() {
	loginCheck1();
	clear();
	var p = $('#operatedg').datagrid('getPager');
	$(p).pagination({
		pageNumber : 1,
		pageSize : 15,
		pageList : [15,20,25],
...
function findList(page,rows) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			$('#operatedg').datagrid('loaded');
			$("#operatedg").datagrid('loadData', obj);
			}
		},
		error : function(err) {
...
function delsetting() {
	var row = $("#operatedg").datagrid("getSelected");
	if (row) {
		var content;
		...
		$.messager.confirm('请确认', content, function(b) {
			if (b) {
				$.ajax({
					...
					success : function(data) {
						if (data.status == "succeed") {
							$("#operatedg").datagrid('unselectAll');
							$('#operatedg').datagrid('getPager').pagination('select');
							$.messager.show({
								timeout : 500,
								msg : '删除成功',
								title : '提示',
							});
							insertLog(adminid, 1, "删除参数：" + key, "运营参数");
						} else {
							$("#operatedg").datagrid('rejectChanges');
		...
		$.ajax({
			...
			success : function(data) {
				if (data.status == "succeed") {
					...
					//插入日志
					insertLog(adminid, 1, "新增参数项:" + settingkey, "运营参数");
				} else {
					$('#operatedg').datagrid('loaded');
	...
	$.ajax({
		...
		success : function(data) {
			if (data.status =="succeed") {
				...
				//保存日志
				insertLog(adminid, 1, "修改参数:"+key, "运营参数管理");
			} else {
				$('#operatedg').datagrid('loaded');
				$.messager.alert('提示', data.msg, 'error');
			}
