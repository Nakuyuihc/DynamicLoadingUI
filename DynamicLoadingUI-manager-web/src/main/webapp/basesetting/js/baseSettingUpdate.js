$(document).ready(function() {
	loginCheck1();
	clear();
	var p = $('#settingdg').datagrid('getPager');
	$(p).pagination({
		pageNumber : 1,
		pageSize : 15,
		pageList : [15,20,25],
		...
		$.messager.confirm('请确认', content, function(b) {
			if (b) {
				$.ajax({
					...
					success : function(data) {
						if (data.status == "succeed") {
							$("#settingdg").datagrid('unselectAll');
							...
							insertLog(adminid, 1, "删除参数：" + key, "系统参数");
						} else {
							$("#settingdg").datagrid('rejectChanges');
		...
		$.ajax({
			...
			success : function(data) {
				if (data.status == "succeed") {
					...
					//插入日志
					insertLog(adminid, 1, "新增参数项:" + settingkey, "系统参数");
				} else {
					$('#settingdg').datagrid('loaded');
	...
	$.ajax({
		...
		success : function(data) {
			if (data.status =="succeed") {
				...
				//保存日志
				insertLog(adminid, 1, "修改参数:"+key, "系统参数管理");
			} else {
				$('#settingdg').datagrid('loaded');
				$.messager.alert('提示', data.msg, 'error');
			}
