$(document).ready(function() {
	loginCheck1();
	clear();
	var p = $('#supsettingdg').datagrid('getPager');
	$(p).pagination({
		pageNumber : 1,
		pageSize : 15,
		pageList : [ 15, 20, 25 ],
	...
	$.ajax({
		...
		success : function(data) {
			if (data.status == "succeed") {
				...
				$('#supsettingdg').datagrid('loaded');
				$("#supsettingdg").datagrid('loadData', obj);
			}
		},
		error : function(err) {
...
function delsetting() {
	var row = $("#supsettingdg").datagrid("getSelected");
	if (row) {
		var content;
		...
		$.messager.confirm('请确认', content, function(b) {
			if (b) {
				$.ajax({
					...
					success : function(data) {
						if (data.status == "succeed") {
							$("#supsettingdg").datagrid('unselectAll');
							$('#supsettingdg').datagrid('getPager').pagination('select');
							$.messager.show({
								timeout : 500,
								msg : '删除成功',
								title : '提示',
							});
							insertLog(adminid, 1, "删除参数：" + key, "超管参数");
						} else {
							$("#supsettingdg").datagrid('rejectChanges');
...
function settingitem() {
	...
	var nickname;
	if (row.nickname == null || row.nickname == 0) {
		nickname = "系统初始化数据"
	} else {
	...
	$.ajax({
		...
		success : function(data) {
			if (data.status == "succeed") {
				...
				//插入日志
				insertLog(adminid, 1, "新增参数项:" + settingkey, "超管参数");
			} else {
				$('#supsettingdg').datagrid('loaded');
	...
	$.ajax({
		...
		success : function(data) {
			if (data.status == "succeed") {
				...
				//保存日志
				insertLog(adminid, 1, "修改参数:" + key, "超管参数管理");
			} else {
				$('#supsettingdg').datagrid('loaded');
				$.messager.alert('提示', data.msg, 'error');
			}
