$(document).ready(function() {
	loginCheck1();
	cleaninfo();
	var p = $('#alliancedg').datagrid('getPager');
	$(p).pagination({
		pageNumber : 1,
		pageSize : 20,
		pageList : [20,30,40 ],
		...
		onSelectPage : function(pageNumber, pageSize) {
			findAllianceDList(pageNumber, pageSize, stratDate, endDate,status);
		},
	});
	box();
	findAllianceDList(1, 20, stratDate, endDate,status);
...
var lognote;
// 分页查询
function findAllianceDList(page, rows, stratDate, endDate,status) {
	$('#alliancedg').datagrid('loading');
	$.ajax({
		...
		success : function(data) {
			if (data.status == "succeed") {
				...
				$('#alliancedg').datagrid('loaded');
				$("#alliancedg").datagrid('loadData', obj);
			} else {
...
function formatterCZ(value, row, index) {
	if (row.status == 1) {
		...
		return '<a href="javascript:transferAccounts();" class="audit">转账成功</a>&nbsp;&nbsp;&nbsp;&nbsp'+
...
function pass() {
	var row = $("#alliancedg").datagrid("getSelected");
	if (row) {
		$.messager.confirm('请确认', '确认通过该提现申请吗?',function(b) {
			if (b) {
				...
				lognote="审核加盟商提现申请:通过申请";
				updateStatus(id,"","审核通过",statusBefore,statusAfter,applyId,lognote);
			}
		});
	} else {
...
function saveNopass() {
	...
	lognote="审核加盟商提现申请:不通过申请,原因"+description;
	...
	if (row) {
		$.messager.confirm('请确认', '确认已转账完成了?', function(b) {
			if (b) {
				...
				statusAfter = 6;
				applyId = row.applyAccountId;
				lognote="审核加盟商提现申请:已转账完成";
				updateStatus(id,"转账成功","",statusBefore,statusAfter,applyId,lognote);
			}
		});
	} else {
		$.messager.alert('提示', '请选择需要审核的数据列', 'warning');
...
function updateStatus(id,comment,description,statusBefore,statusAfter,applyId,lognote) {
	$.ajax({
		...
		success : function(data) {
			if (data.status == "succeed") {
				$('#alliancedg').datagrid('getPager').pagination('select');
				//插入日志
				insertLog(adminid, 1, lognote, "加盟商提现申请");
...
function box() {
	...
	findAllianceDList(page, rows, stratDate, endDate,status);
}