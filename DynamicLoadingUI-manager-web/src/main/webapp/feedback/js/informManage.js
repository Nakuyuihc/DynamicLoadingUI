$(document).ready(function() {
	//loginCheck1();
	var p = $('#snsReport').datagrid('getPager');
	$(p).pagination({
		pageNumber : 1,
		pageSize : 20,
		pageList : [ 20, 30, 40 ],
...
function findFbList(staTime, endTime, status, page, rows) {
	...
	feefbackService.findList(staTime, endTime, status, page, rows, function(data) {
		if (data.status == "succeed") {
			...
			$('#snsReport').datagrid('loaded');
			$("#snsReport").datagrid('loadData', pageObjFb);
		} else {
...
/*function updateStatus() {
	var row = $("#snsReport").datagrid("getSelected");
	if (row == undefined || row == '' || row == null) {
		$.messager.alert('提示', '请选择需要操作的举报信息', 'warning');
		return;
	}
	let id = row.id;
	var feefbackService = new FeefbackService();
...
function FeefbackService() {
	// 请求url
	this.selectUrl = "system/sns/snsReportList";
	/*this.updateUrl = "system/sns/updateSnsReportStatus";*/
}
;
