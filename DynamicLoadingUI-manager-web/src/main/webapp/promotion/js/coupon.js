...
function saveInfo() {
	/*// 判断表单是否通过验证
	var valid = $('#coupon_from').form('validate');
	if (!valid) {
		return valid;
	}*/
	$("#btnID").trigger("click");
	...
	// 获取表单af中的值并转化
	/*var afInfo = $("#coupon_from").serializeArray();*/
	var name = $("#name").val();
	...
	$.ajax({
		...
		success : function(data) {
			if (data.status == "succeed") {
				...
				//插入日志
				insertLog(adminid, 1, "新增优惠券:" + name, "优惠券");
			} else {
				$('#coupondg').datagrid('loaded');
...
function showDetail() {
	...
	if (row == undefined || row == '' || row == null) {
		$.messager.alert('提示', '请选择需要查看的优惠券！', 'warning');
		return;
	}
...
function deleted() {
	...
	var row = $("#coupondg").datagrid("getSelected");
	if (row) {
		$.messager.confirm('请确认', '确定要弃用这个优惠券吗？', function(b) {
			if (b) {
				id = row.id;
				$.ajax({
					...
					success : function(data) {
						if (data.status == "succeed") {
							...
							insertLog(adminid, 1, "删除优惠券：" + row.name, "优惠券");
						} else {
